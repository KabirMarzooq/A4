...
import { User, Calendar, Clock, Check, X, Search, Inbox } from "lucide-react";
  ...
  return (
    <div className="p-2 transition-colors duration-300">
      ...
      <div className="flex gap-2 border-b border-slate-200 dark:border-slate-700 mb-6">
        {[
          { id: "queue", label: "Incoming Requests" },
          { id: "sync", label: "Online Bookings", count: syncPendingCount },
        ].map((tab) => (
          <button
            key={tab.id}
      ...
      {activeTab === "queue" ? (
        ...
        <DoctorSyncRequestsTab onPendingCountChange={setSyncPendingCount} />
      )}
    </div>
  );
}

// ── MY QUEUE (walk-in / directly-booked local appointments) ────────────────
function MyQueueTab() {
...
// ── ONLINE BOOKINGS (cloud-synced requests addressed to this doctor) ───────
// The backend already scopes GET /sync-requests to this doctor's own name
// for a doctor role (see SyncedBookingRequestController::index) — reception
// sees the same queue but read-only, this is where the actual decision
// happens.
function DoctorSyncRequestsTab({ onPendingCountChange }) {
  ...
  const fetchRequests = async () => {
    try {
      ...
      toast.error("Error loading online bookings.");
    } finally {
      setLoading(false);
    }
  };
      ...
      {filteredRequests.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-slate-400 text-sm mt-1">
            Patients booking you online appear here automatically.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
          {filteredRequests.map((r) => (
            ...
            >
              ...
              {r.status === "pending" ? (
                ...
                <div className="w-full py-3 bg-slate-50 dark:bg-slate-900/50 text-slate-400 font-bold rounded-xl text-center text-xs border border-slate-100 dark:border-slate-700 uppercase tracking-widest">
                  {r.status === "confirmed"
                    ? "Confirmed — reception will register the patient"
                    : "Action Completed"}
                </div>
              )}
            </div>
